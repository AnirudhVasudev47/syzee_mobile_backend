...
exports.productsSubCategory = (req, res) => {
  ...
  form.parse(req, (err, fields) => {
    ...
    if (mainCatId == 1) { // Women Category
      const womenQuery = 'SELECT `women`.`ID`, `women`.`PRODUCT_ID` AS `id`, `women`.`NAME` AS `name`, `women`.`IMAGE` AS `image`, `women`.`VARIANTS` AS `price`, `women`.`CREATED_ON` AS `created_on`, `women`.`RATINGS` AS `rating`, `women`.`COLOUR` AS `color`, `brands`.`NAME` AS `brand` FROM `women` LEFT JOIN `brands` ON `women`.`BRAND_ID` = `brands`.`ID` WHERE `women`.`CATEGORY_ID`=?'
      ...
      connection.query(womenQuery, subCategoryId, (err, womenResults) => {
        ...
        if (!email) {
          ...
          connection.query(wishlistQuery, email, (err, wishRes) => {
            ...
            womenResults.map((prod) => {
              try {
                const checkProdId = obj => obj.PRODUCT_ID === prod.ID;
      ...
      const kidsQuery = 'SELECT `kids`.`ID` AS `id`, `kids`.`NAME` AS `name`, `kids`.`IMAGE` AS `image`, `kids`.`VARIANTS` AS `var`, `kids`.`CREATED_ON` AS `created_on`, `kids`.`RATINGS` AS `rating`, `kids`.`COLOUR` AS `color`, `brands`.`NAME` AS `brand` FROM `kids` LEFT JOIN `brands` ON `kids`.`BRAND_ID` = `brands`.`ID` WHERE `kids`.`CATEGORY_ID`=?'
      ...
      const menQuery = 'SELECT `men`.`ID` AS `id`, `men`.`NAME` AS `name`, `men`.`IMAGE` AS `image`, `men`.`VARIANTS` AS `var`, `men`.`CREATED_ON` AS `created_on`, `men`.`RATINGS` AS `rating`, `men`.`COLOUR` AS `color`, `brands`.`NAME` AS `brand` FROM `men` LEFT JOIN `brands` ON `men`.`BRAND_ID` = `brands`.`ID` WHERE `men`.`CATEGORY_ID`=?'
...
exports.getWomenNewIn = (req, res) => {
  ...
  form.parse(req, (err, fields) => {
    ...
    if (mainCatId == 1) { // Women Category
      ...
      const kidsQuery = 'SELECT `kids`.`ID` AS `id`, `kids`.`NAME` AS `name`, `kids`.`IMAGE` AS `image`, `kids`.`VARIANTS` AS `price`, `kids`.`CREATED_ON` AS `created_on`, `kids`.`RATINGS` AS `rating`, `kids`.`COLOUR` AS `color`, `brands`.`NAME` AS `brand` FROM `kids` LEFT JOIN `brands` ON `kids`.`BRAND_ID` = `brands`.`ID` ORDER BY `kids`.`NA_DATE` DESC'
      ...
      const menQuery = 'SELECT `men`.`ID` AS `id`, `men`.`NAME` AS `name`, `men`.`IMAGE` AS `image`, `men`.`VARIANTS` AS `price`, `men`.`CREATED_ON` AS `created_on`, `men`.`RATINGS` AS `rating`, `men`.`COLOUR` AS `color`, `brand`.`NAME` AS `brand` FROM `men` LEFT JOIN `brands` ON `men`.`BRAND_ID` = `brands`.`ID` ORDER BY `men`.`NA_DATE` DESC'
